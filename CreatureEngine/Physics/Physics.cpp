...
template<typename Object_Type> Simulation<Object_Type>::Simulation(double _timestep, double _gravity)
{
   // SpatialTree = new QuadTree({ 0,0 }, { 2000,2000 });
}
template<typename Object_Type> void Simulation<Object_Type>::Update(float _timestep)
{// Update each of the Rigidbodies in our simulation with the given timestep
    ...
    {// Crude O(log(N)) Test for Collisions. Replace with Quad tree // *&SpatialTree->RootNode->QueryRange(C->g_Position(), { 100,100 }));
        std::vector<Object_type*> EMPTYVEC;
        C->g_Collider().Sweep(EMPTYVEC);
...
template<typename Object_Type> void Simulation<Object_Type>::Resolve()
...
template<typename Object_Type> void Simulation<Object_Type>::Render()
...
template<typename Object_Type> void Simulation<Object_Type>::Add(Object_type *_object)
{// Add an Object to our physics simulation
    Colliders.push_back(_object);
}
template<typename Object_Type> void Simulation<Object_Type>::Remove(Object_type *_deadObject)
...
template<typename Object_Type> void Simulation<Object_Type>::Clean()
{
...
template<typename Object_Type> void Simulation<Object_Type>::Add_Object(Object_type *_object)
{
