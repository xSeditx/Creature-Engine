...
Collider::Collider(Parent_t* _parent, UserCollisionResponse _response, float _weight)
	:
	Parent(_parent),
	Weight(_weight),
	Velocity(NULL),
...
void Collider::Respond(Parent_t& _other)
{
...
SphereCollider::SphereCollider(Parent_t* _parent, Vec2 _position, float _radius)
	:
...
AABBCollider::AABBCollider(Parent_t* _parent)
//	:
...
AABBCollider::AABBCollider(Parent_t* _parent, Vec2 _position, Vec2 _size)
	:
...
void Collider::s_Parent(Parent_t* _parent)
{
	Parent = _parent;
}
...
{// Checks to see if a point is within the bounds of the AABB axes
	...
	return
		(_point.x >= std::min(x1, x2) && _point.x <= std::max(x1, x2)) &&
		(_point.y >= std::min(y1, y2) && _point.y <= std::max(y1, y2));
...
{// Check to see if the Closest point on an AABB is less than Radius of the Sphere
	Vec2 Topleft = (Position - (Size * 0.5f));
	Vec2 Min = Vec2(std::min(Topleft.x, (Topleft.x + Size.x)), std::min(Topleft.y, (Topleft.y + Size.y)));
	Vec2 Max = Vec2(std::max(Topleft.x, (Topleft.x + Size.x)), std::max(Topleft.y, (Topleft.y + Size.y)));

	float x = std::max(Min.x, std::min(_other->Position.x, Max.x));
	float y = std::max(Min.y, std::min(_other->Position.y, Max.y));
...
Vec4 Collider::Overlap(Collider* _other)
{
    ...
    Vec2 O = Vec2(0);/// Camera::get().g_Center();
...
SphereCollider::SphereCollider(Parent_t* _parent)
    :
    ///SphereCollider
    ///(
    ///	_parent,
    ...
    ///)
    Collider(_parent,  Default_Response, 10.0f)
{
}
...
bool Collider::Sweep(std::vector<Parent_t*>& _test)
{/// We should return the Collision Pair in the future but this is good enough for now
    if (is_Alive() _EQUALS_ false)
    {// if it is not even Alive get the hell out of here
        return {};
    }
